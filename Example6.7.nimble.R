...
logistic_impute=nimbleCode(
{
# this model estimates the parameters of a logistic model
# for missing values;
#  MODEl   {                       
                ...
                # complete-data likelihood;
                        for( i in 1 : M ) {
                                y_miss[i] ~ dbern(p[i]);
                                logit(p[i]) <-beta0+beta1*x1_miss[i]+beta2*x2_miss[i];
                                x1_miss[i] ~ dnorm(mu, tau);
                                x2_miss[i] ~ dnorm(theta[i], psi);
                                theta[i] <- alpha0+alpha1*x1_miss[i]; 
                                y_impute[i] <- y_miss[i];
                                x1_impute[i] <- x1_miss[i];
                                x2_impute[i] <- x2_miss[i];                             
                                }
...
# initial values
# INITIAL VALUES list(beta0=-1, beta1=0.5, beta2=0.5; mu=0, alpha0=0, alpha1=0, tau=1, psi=1)
...
logistic.constants=list(M=rowobs);

logistic.data=list(y_miss=y_miss, x1_miss=x1_miss, x2_miss=x2_miss);

logistic.inits=list(beta0=-1, beta1=0.5, beta2=0.5, mu=0, alpha0=0, alpha1=0, tau=1, psi=1);


# compiling;
logistic_impute_model=nimbleModel(logistic_impute, constants=logistic.constants, 
data=logistic.data, inits=logistic.inits);

logistic.configure=configureMCMC(logistic_impute_model);

logistic.configure$resetMonitors();

logistic.configure$addMonitors('x1_impute', 'x2_impute', 'y_impute');


gibbs_no=10000;
burnin=gibbs_no/2;

logistic.configure$setThin(burnin/mi_no);

logistic.mcmc=buildMCMC(logistic.configure);

logistic_impute_compiled=compileNimble(logistic_impute_model, logistic.mcmc);

logistic_impute_compiled$logistic.mcmc$run(gibbs_no);

results=as.matrix(logistic_impute_compiled$logistic.mcmc$mvSamples);
...
# rm(logistic_impute_model)
# rm(logistic_impute_compiled);

# process_stage2_data_simulate=list(M=M, y_miss=y_miss, x1_miss=x1_miss, x2_miss=x2_miss);
# process_stage2_init= function(){
# list(beta0=-1, beta1=0.5, beta2=0.5, mu=0, alpha0=0, alpha1=0, tau=1, psi=1)};
...
x1_completed_bugs=results_final[i,1:rowobs];
x2_completed_bugs=results_final[i,(rowobs+1):(2*rowobs)];
y_completed_bugs=results_final[i,(2*rowobs+1):(3*rowobs)];
