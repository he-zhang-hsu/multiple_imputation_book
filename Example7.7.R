...
bugs_mean_mat=bugs_mean_var_mat=glm_mean_mat=glm_mean_var_mat=mice_mean_mat=mice_mean_var_mat=matrix(NA, cycle_no, mi_no);
...
process_stage2_data_simulate=list("M","y_miss", "x1_miss", "x2_miss");
...
ori_data_more=data.frame(as.factor(y_miss), x1_miss, x2_miss);
...
y_completed_mice=as.numeric(imp_data_more_final[,1])-1;
...
# marginal means;
mice_mean=mean(y_completed_mice);
mice_var=var(y_completed_mice)/rowobs;

mice_mean_mat[cycle,i]=mice_mean;
mice_mean_var_mat[cycle,i]=mice_var;

# logistic regression coefficient for x1 slope;
mice_logistic=summary(glm(y_completed_mice~x1_completed_mice+x2_completed_mice, family=binomial(link="logit")));
...
# FCS imputation by R MICE;
mice_MI=mi_performance(mice_mean_mat, mice_mean_var_mat, true_mean, rowobs);
mice_MI;
