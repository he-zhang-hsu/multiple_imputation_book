...
# The full dataset of missing gestational age;
...
# Summary statistics of variables involved;
# Table 6.3
...
summary(dbirwt);
summary(frace);
table(frace, useNA="always")/sum(!is.na(frace));
table(mrace, useNA="always")/sum(!is.na(mrace));
table(meduc, useNA="always")/sum(!is.na(meduc));



# collapse the two less than high school into one group
# for general location model imputation;
...
# square root transformation of DBIRWT;
...
# form the worling dataset that
# only contains the five variables;
...
# using the square root transformation of DBIRWT
# SQRTMI method
...
# The columns include the regression estimates, their 95% CI and standard errors;

sat_summary_matrix;


# Genearl location model imputation;
# imputation using the linear term of dbirwt;
# LINMI method
