...
# calculate the design effect using final MI estimates;
# for proportions;
...
ylec_completed_x=(y_completed_x <=c);

# marginal mean;
mi_x.data=as.data.frame(cbind(y_completed_x, ylec_completed_x, wt_sample));
mi_x=svydesign(id=~1, weights=wt_sample, data=mi_x.data);
# q.mi_x=svymean(~y_completed_x, mi_x, deff=TRUE);
q.mi_x=svymean(~ylec_completed_x, mi_x, deff=TRUE);
...
ylec_completed_xw=(y_completed_xw <=c);

# marginal mean;
mi_xw.data=as.data.frame(cbind(y_completed_xw, ylec_completed_xw, wt_sample));
mi_xw=svydesign(id=~1, weights=wt_sample, data=mi_xw.data);
# q.mi_xw=svymean(~y_completed_xw, mi_xw, deff=TRUE);
q.mi_xw=svymean(~ylec_completed_xw, mi_xw, deff=TRUE);
...
mean(mi_deff(MI_X_mean_mat, MI_X_mean_var_mat, true_mean, rowobs));
mean(rowobs/mi_deff(MI_X_mean_mat, MI_X_mean_var_mat, true_mean, rowobs));
...
mean(mi_deff(MI_XW_mean_mat, MI_XW_mean_var_mat, true_mean, rowobs));
mean(rowobs/mi_deff(MI_XW_mean_mat, MI_XW_mean_var_mat, true_mean, rowobs));
...
mean(1/(1-mi_fmi(MI_X_mean_mat, MI_X_mean_var_mat, true_mean, rowobs))
*rowMeans(MI_X_deff_mat));

mean(rowobs/(1/(1-mi_fmi(MI_X_mean_mat, MI_X_mean_var_mat, true_mean, rowobs))
*rowMeans(MI_X_deff_mat)));
...
mean(1/(1-mi_fmi(MI_XW_mean_mat, MI_XW_mean_var_mat, true_mean, rowobs))
*rowMeans(MI_XW_deff_mat));

mean(rowobs/(1/(1-mi_fmi(MI_XW_mean_mat, MI_XW_mean_var_mat, true_mean, rowobs))
*rowMeans(MI_XW_deff_mat)));
