...
y2_imputed_draw=rep(NA, mis_no);
obs_pred_mean=cbind(cbind(1, x_obs, u_obs, z_obs)%*%regcoef,y2_obs);
...
y2_imputed_draw[k]=sample(donor_matrix_sorted[1:donor_no,2],1);
}
...
y2_imputed_mle=mice.impute.norm(y2_miss, ry=as.logical(1-miss_indi), seed=i, x=cbind(x,u,z));
y2_imputed_mle[y2_imputed_mle >=0.5]=1;
y2_imputed_mle[y2_imputed_mle < 0.5]=0;
...
y2_imputed_boot=mice.impute.pmm(y2_miss, ry=as.logical(1-miss_indi), seed=i, x=cbind(x,u,z));
...
hist(coef_s1x_draw_collection[,2], main="", xlab="");
