...
bugs_mean_mat=bugs_var_mat=glm_mean_mat=glm_var_mat=mice_mean_mat=mice_var_mat=matrix(NA, cycle_no, mi_no);

BD_slope_vec=BD_slope_var_vec=rep(NA, cycle_no);
CC_slope_vec=CC_slope_var_vec=rep(NA, cycle_no);
bugs_slope_mat=bugs_slope_var_mat=glm_slope_mat=glm_slope_var_mat=mice_slope_mat=mice_slope_var_mat=matrix(NA, cycle_no, mi_no);


# generate data for example 9.5
...
process_stage2.sim <- bugs(data=process_stage2_data_simulate, inits=process_stage2_init, parameters=process_stage2_parameters, model.file="C:/Users/WDQ7/book_program/logitmodel_missing_bivariate_forR.txt",
...
# mice imputation;
ori_data_more=data.frame(as.factor(y2_miss), x1_miss, x2_miss);


for (i in 1:mi_no)
{

imp_data_more=mice(ori_data_more, m=1, seed=i, maxit=20, meth=c('logreg','norm','norm'));
imp_data_more_final=complete(imp_data_more);

y2_completed_mice=as.numeric(imp_data_more_final[,1])-1;
x1_completed_mice=imp_data_more_final[,2];
x2_completed_mice=imp_data_more_final[,3];


# multiple imputation analysis;

# marginal means;
mice_mean=mean(y2_completed_mice);
mice_var=var(y2_completed_mice)/rowobs;

mice_mean_mat[cycle,i]=mice_mean;
mice_var_mat[cycle,i]=mice_var;

# logistic regression coefficient for x1 slope;
mice_logistic=summary(glm(y2_completed_mice~x1_completed_mice+x2_completed_mice, family=binomial(link="logit")));
mice_slope_mat[cycle,i]=mice_logistic$coeff[2,1];
mice_slope_var_mat[cycle,i]=mice_logistic$coeff[2,2]^2;
...
mice_mi_mean=rep(NA, cycle_no);
mice_mi_var=rep(NA, cycle_no);
mice_mi_df=rep(NA, cycle_no);
mice_mi_f=rep(NA, cycle_no);
...
mice_summary=pool.scalar(mice_mean_mat[i,], mice_var_mat[i,], n=rowobs);
mice_mi_mean[i]=mice_summary$qbar;
mice_mi_var[i]=mice_summary$t;
mice_mi_df[i]=mice_summary$df;
mice_mi_f[i]=mice_summary$f;
...
# mice imputation;
mice_mi_true=mean(mice_mi_mean);
mice_mi_bias=mice_mi_true-true;

mice_mi_bias;
mice_mi_bias/true;

mice_mi_mse=mean((mice_mi_mean-true)^2);
mice_mi_mse;

# coverage;
mice_mean_low95_vec=mice_mi_mean-qt(.975, mice_mi_df)*sqrt(mice_mi_var);
mice_mean_up95_vec=mice_mi_mean+qt(.975, mice_mi_df)*sqrt(mice_mi_var);

# trans_mean_low95_vec=trans_mean_low95_vec[!is.na(trans_mean_low95_vec)];
# trans_mean_up95_vec=trans_mean_up95_vec[!is.na(trans_mean_up95_vec)];


mean_mice_length=mean(mice_mean_up95_vec-mice_mean_low95_vec);
mean_mice_length;

mice_mi_coverage=(mice_mean_low95_vec < true)*(mice_mean_up95_vec > true);
mean(mice_mi_coverage);

# variance estimates;
mean(sqrt(mice_mi_var));
sqrt(var(mice_mi_mean));
...
mice_summary=pool.scalar(mice_slope_mat[i,], mice_slope_var_mat[i,], n=rowobs);
mice_mi_mean[i]=mice_summary$qbar;
mice_mi_var[i]=mice_summary$t;
mice_mi_df[i]=mice_summary$df;
mice_mi_f[i]=mice_summary$f;
...
mice_mi_bias=mean(mice_mi_mean)-true_slope;
mice_mi_bias;

mice_mi_bias/true_slope;

mice_mi_mse=mean((mice_mi_mean-true_slope)^2);
mice_mi_mse;

# coverage;
mice_slope_low95_vec=mice_mi_mean-qt(.975, mice_mi_df)*sqrt(mice_mi_var);
mice_slope_up95_vec=mice_mi_mean+qt(.975, mice_mi_df)*sqrt(mice_mi_var);
