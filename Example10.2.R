...
# nomalize the selection probability
...
# Vectors and matrices holding estimates;
...
# linear model imputation using design variable as the predictor;
...
# marginal mean;
mi_z.data=as.data.frame(cbind(y_completed_z, wt_sample));
mi_z=svydesign(id=~1, weights=wt_sample, data=mi_z.data);
q.mi_z=svymean(~y_completed_z, mi_z, deff=TRUE);
...
# marginal mean;
mi_zw.data=as.data.frame(cbind(y_completed_zw, wt_sample));
mi_zw=svydesign(id=~1, weights=wt_sample, data=mi_zw.data);
q.mi_zw=svymean(~y_completed_zw, mi_zw, deff=TRUE);
...
# End of the multiple imputation;
...
matplot(z_sample[1:250], y_plot[1:250,], pch=c(1,16), xlab="z", ylab="y");
...
# matplot(y_plot[1:250,], z[1:250], pch=c(1,16), zlab="y", ylab="z");
...
# The 2nd row;
matplot(z_sample[1:250], y_plot_impute[1:250,], pch=c(1,17), xlab="z", ylab="y");
...
# matplot(z_sample[1:250], y_plot_impute[1:250,], pch=c(1,15), xlab="z", ylab="y");
...
y_plot_imputed=y_completed_z;
y_plot_imputed[miss_indi==0]=NA;
y_plot_impute=cbind(y_plot_obs, y_plot_imputed);


# The 3rd row;
matplot(z_sample[1:250], y_plot_impute[1:250,], pch=c(1,18), xlab="z", ylab="y");
...
matplot(z_sample[1:250], y_plot_impute[1:250,], pch=c(1,15), col=c("black", "red"), bg="red", xlab="z", ylab="y");
...
matplot(z_sample[1:250], y_plot_impute[1:250,], pch=c(1,25), col=c("black", "red"), bg="red", xlab="z", ylab="y");

matplot(wt_sample[1:250], y_plot_impute[1:250,], pch=c(1,25), col=c("black", "red"), bg="red", xlab="weight", ylab="y");

# matplot(1/wt_sample[1:250], y_plot_impute[1:250,], pch=c(1,25), col=c("black", "red"), bg="red", zlab="selection prob", ylab="y");
